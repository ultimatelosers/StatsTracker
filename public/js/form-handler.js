...
function fetchPlayerStats() {
  fetch('/.netlify/functions/get-player-stats')
    ...
    .then(data => {
      playerData = getMostRecentPlayerStats(data); // Store globally
      renderStatsTable(playerData); // Initial render
...
function renderStatsTable(playerData) {
  ...
  if (currentSortColumn) {  // Check if a sort column is selected
    playerData.sort((a, b) => {
  ...
  playerData.forEach(player => {
    ...
    const parsedDate = new Date(player.created_at);
